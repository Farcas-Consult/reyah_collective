...
export default function SupplierOrdersPage() {
  ...
  const loadOrders = () => {
    ...
    if (storedOrders && storedProducts) {
      ...
      const supplierProducts = allProducts.filter((p: any) => p.supplier === user?.companyName)
      
      // Filter orders that contain at least one item from supplier's products
      const supplierOrders = allOrders
        .map((order: Order) => {
          ...
          const supplierItems = order.items.filter(item => {
            const product = supplierProducts.find((p: any) => p.name === item.name)
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          <div className="flex items-center gap-3 mb-2">
            <h1 className="text-3xl font-bold text-[var(--brown-800)]">Supply Orders</h1>
            <span className="px-3 py-1 bg-indigo-600 text-white text-xs font-bold rounded-full">
              SUPPLIER
            </span>
          </div>
          <p className="text-gray-600">Track and manage orders for your supplied products</p>
        </div>
